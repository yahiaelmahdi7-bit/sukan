import { ImageResponse } from "next/og";
import { getListingById, getLocaleTitle } from "@/lib/sample-listings";
import type { Locale } from "@/i18n/routing";

export const size = { width: 1200, height: 630 };
export const contentType = "image/png";

const STATE_LABELS: Record<string, string> = {
  khartoum: "Khartoum",
  al_jazirah: "Al Jazirah",
  blue_nile: "Blue Nile",
  sennar: "Sennar",
  white_nile: "White Nile",
  north_kordofan: "North Kordofan",
  south_kordofan: "South Kordofan",
  west_kordofan: "West Kordofan",
  north_darfur: "North Darfur",
  south_darfur: "South Darfur",
  east_darfur: "East Darfur",
  central_darfur: "Central Darfur",
  west_darfur: "West Darfur",
  kassala: "Kassala",
  red_sea: "Red Sea",
  gedaref: "Gedaref",
  river_nile: "River Nile",
  northern: "Northern",
};

const STATE_LABELS_AR: Record<string, string> = {
  khartoum: "الخرطوم",
  al_jazirah: "الجزيرة",
  blue_nile: "النيل الأزرق",
  sennar: "سنار",
  white_nile: "النيل الأبيض",
  north_kordofan: "شمال كردفان",
  south_kordofan: "جنوب كردفان",
  west_kordofan: "غرب كردفان",
  north_darfur: "شمال دارفور",
  south_darfur: "جنوب دارفور",
  east_darfur: "شرق دارفور",
  central_darfur: "وسط دارفور",
  west_darfur: "غرب دارفور",
  kassala: "كسلا",
  red_sea: "البحر الأحمر",
  gedaref: "القضارف",
  river_nile: "نهر النيل",
  northern: "الشمالية",
};

export default async function ListingOgImage({
  params,
}: {
  params: Promise<{ id: string; locale: string }>;
}) {
  const { id, locale: localeParam } = await params;
  const listing = getListingById(id);
  const locale = localeParam as Locale;
  const isAr = locale === "ar";

  const title = listing
    ? getLocaleTitle(listing, locale)
    : isAr
      ? "عقار"
      : "Property";

  const stateLabel = listing
    ? (isAr
        ? (STATE_LABELS_AR[listing.state] ?? listing.state)
        : (STATE_LABELS[listing.state] ?? listing.state))
    : "";

  const priceDisplay = listing
    ? `$${listing.priceUsd.toLocaleString("en-US")} / ${listing.period}`
    : "";

  return new ImageResponse(
    (
      <div
        style={{
          width: 1200,
          height: 630,
          background: "#12100C",
          display: "flex",
          flexDirection: "column",
          justifyContent: "space-between",
          padding: "56px 72px",
          position: "relative",
          overflow: "hidden",
        }}
      >
        {/* Background haze */}
        <div
          style={{
            position: "absolute",
            top: -100,
            right: -100,
            width: 480,
            height: 480,
            borderRadius: "50%",
            background:
              "radial-gradient(circle, rgba(200,135,58,0.15) 0%, transparent 65%)",
          }}
        />

        {/* Top row: mark + brand name */}
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <svg
            width={52}
            height={52}
            viewBox="0 0 64 64"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <defs>
              <linearGradient
                id="g"
                x1="8"
                y1="8"
                x2="56"
                y2="56"
                gradientUnits="userSpaceOnUse"
              >
                <stop offset="0%" stopColor="#C8873A" />
                <stop offset="100%" stopColor="#E0A857" />
              </linearGradient>
            </defs>
            <path
              d="M8 15 C16 9,26 9,32 14 C38 19,48 22,56 17 C54 19,48 24.5,40 21.5 C32 18.5,22 18,14 21 C10 22.5,8 20,8 18 Z"
              fill="url(#g)"
            />
            <path
              d="M8 27 C16 22,26 21,32 26 C38 31,49 33,56 28.5 C54 30.5,48 35.5,40 33 C32 30.5,22 30,14 33 C10 34.5,8 32,8 30 Z"
              fill="url(#g)"
            />
            <path
              d="M8 39 C16 34,26 33.5,32 38 C38 42.5,49 44.5,56 40.5 C54.5 42.5,48 46.5,40 44.5 C32 42.5,22 42,14 44.5 C10 45.5,8 43.5,8 42 Z"
              fill="url(#g)"
            />
            <circle cx="24" cy="52" r="2" fill="#D4943F" />
            <circle cx="32" cy="52" r="2" fill="#D4943F" />
            <circle cx="40" cy="52" r="2" fill="#D4943F" />
          </svg>
          <span
            style={{
              fontFamily: "Georgia, serif",
              fontSize: 28,
              color: "rgba(253,248,240,0.7)",
              letterSpacing: "1px",
            }}
          >
            Sukan
          </span>
        </div>

        {/* Center: listing title */}
        <div
          style={{
            display: "flex",
            flexDirection: "column",
            gap: 16,
            flex: 1,
            justifyContent: "center",
          }}
        >
          <div
            style={{
              fontSize: listing && title.length > 50 ? 44 : 56,
              fontFamily: "Georgia, 'Times New Roman', serif",
              color: "#FDF8F0",
              lineHeight: 1.2,
              maxWidth: 960,
            }}
          >
            {title}
          </div>

          <div style={{ display: "flex", alignItems: "center", gap: 24, marginTop: 8 }}>
            {stateLabel ? (
              <div
                style={{
                  fontSize: 22,
                  fontFamily: "Georgia, serif",
                  color: "rgba(200,135,58,0.9)",
                }}
              >
                {stateLabel}
              </div>
            ) : null}
            {priceDisplay ? (
              <div
                style={{
                  fontSize: 22,
                  fontFamily: "Georgia, serif",
                  color: "rgba(253,248,240,0.55)",
                }}
              >
                {priceDisplay}
              </div>
            ) : null}
          </div>
        </div>

        {/* Bottom: tagline + URL */}
        <div
          style={{
            display: "flex",
            alignItems: "center",
            justifyContent: "space-between",
          }}
        >
          <div
            style={{
              fontSize: 18,
              fontFamily: "Georgia, serif",
              color: "rgba(253,248,240,0.4)",
              letterSpacing: "0.5px",
            }}
          >
            {isAr ? "بيت السودان للسكن" : "Sudan's home for housing"}
          </div>
          <div
            style={{
              fontSize: 15,
              fontFamily: "Georgia, serif",
              color: "rgba(200,135,58,0.45)",
              letterSpacing: "2px",
              textTransform: "uppercase",
            }}
          >
            sukan.app
          </div>
        </div>
      </div>
    ),
    { ...size },
  );
}
