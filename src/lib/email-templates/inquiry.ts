...
export function buildInquiryEmail(p: InquiryEmailParams): string {
  ...
  // ── Copy ──────────────────────────────────────────────────────────────────
  const title = isAr
    ? `استفسار جديد على ${p.listingTitle} · سُكَن`
  ...
  const subText = isAr
    ? `أرسل <strong>${escHtml(p.inquirerName)}</strong> الرسالة التالية عبر سُكَن بخصوص إعلانك.`
  ...
  const ctaDashboard = isAr ? "فتح لوحة التحكم في سُكَن" : "Open in Sukan dashboard";

  // ── WhatsApp deep-link ────────────────────────────────────────────────────
  const waReplyText = isAr
    ? `أهلاً، أريد الرد على استفسارك عن إعلاني على سُكَن: ${p.listingTitle}`
...
export function inquiryEmailSubject(listingTitle: string, locale: string): string {
  return locale === "ar"
    ? `استفسار جديد على ${listingTitle} · سُكَن`
